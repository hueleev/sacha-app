...
- 등록할 음악,영화,책, 사진의 데이터 정보는 별도의 테이블로 구분하여 입력항목을 저장한다. (\*\*\*\_contents 테이블) 이미 해당 source_id가 있는 경우, contents 테이블에 저장하지 않는다. openAPI로 검색하지 않고, '직접입력'한 경우, prefix로 `taste-` 를 붙여 uuid를 생성하여 source_id를 만들어 insert한다.
- 소감, 별점, 작성일자와 같은 정보는 posts 테이블에 저장하고, \*\*\*\_contents 테이블과 relation을 맺어준다.
...
- 목록
  ...
  5. 본인이 올린 게시글엔 팔로우와 책갈피, 하트가 뜨지 않고, 휴지통(삭제) 아이콘이 뜬다. [ ]
