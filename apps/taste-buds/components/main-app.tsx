...
export default function MainApp({ user, profile, onLogout }: MainAppProps) {
  ...
  const [tastes, setTastes] = useState<TasteItem[]>([
    {
      ...
      author: "user123",
      description:
        "정말 감동적인 뮤지컬 영화였어요. 음악과 영상미가 완벽했습니다.",
      likes: 12,
      isFollowing: false,
      isBookmarked: true,
    },
  ...
  const filteredTastes = tastes
    ...
    .filter((taste, index, array) => {
      ...
      return (
        array.findIndex(
          (t) => t.title === taste.title && t.type === taste.type
        ) === index
      );
    });
  ...
  const toggleFollow = (id: string) => {
    setTastes(
      tastes.map((taste) =>
        taste.id === id ? { ...taste, isFollowing: !taste.isFollowing } : taste
      )
    );
  };
  ...
  const toggleLike = (id: string) => {
    setTastes(
      tastes.map((taste) =>
        taste.id === id ? { ...taste, likes: taste.likes + 1 } : taste
      )
    );
  };
  ...
  const toggleBookmark = (id: string) => {
    setTastes(
      tastes.map((taste) =>
        taste.id === id
          ? { ...taste, isBookmarked: !taste.isBookmarked }
          : taste
      )
    );
  };
      ...
      <Tabs defaultValue="moimoi" className="w-full">
        ...
        <TabsContent value="moimoi" className="mt-0">
          <div className="grid grid-cols-2 border-l border-black">
            {filteredTastes
              .filter((taste) => taste.author === user.name)
              .map(renderGridItem)}
          </div>
        </TabsContent>
        ...
        <TabsContent value="tastopia" className="mt-0">
          <div className="grid grid-cols-2 border-l border-black">
            {filteredTastes.map(renderGridItem)}
          </div>
        </TabsContent>
        ...
        <TabsContent value="favfolk" className="mt-0">
          <div className="grid grid-cols-2 border-l border-black">
            {filteredTastes
              .filter((taste) => taste.isFollowing)
              .map(renderGridItem)}
          </div>
        </TabsContent>
        ...
        <TabsContent value="zzimzzim" className="mt-0">
          <div className="grid grid-cols-2 border-l border-black">
            {filteredTastes
              .filter((taste) => taste.isBookmarked)
              .map(renderGridItem)}
          </div>
        </TabsContent>
      </Tabs>
